	...
	Map<Vocabulary, Integer> vocab;
	 ...
	 @Override
	  public void initialize(final UimaContext context) throws ResourceInitializationException {
		...
		String listFilePath1 = "resources\\frequencyLists\\EVP.csv";
		String listFilePath2 = "resources\\frequencyLists\\ExtendForEVP.csv";
		try {
...
//			String pOS =   t.getPos().getPosValue();
			...
			Vocabulary vocabulary = new Vocabulary(lemma,wordType);
			  ...
			  if (vocab.containsKey(vocabulary)){ 
				  Frequency f = new Frequency(aJCas); 
				  f.setBegin(t.getBegin());
				  f.setEnd(t.getEnd()); 
				  f.setValue(vocab.get(vocabulary));
				  f.setFrequencyBand(computeFrequencyBand(vocab.get(vocabulary)));
				  f.addToIndexes(); 
			  }
		}		
